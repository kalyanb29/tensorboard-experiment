...
import umap
from tensorboard.plugins.uMAP import im_util
from tensorboard.plugins.uMAP.file_system_tools import read_pickle,\
  write_pickle, write_file
from tensorboard.plugins.uMAP.shared_config import PLUGIN_NAME, TAG_NAME,\
  SUMMARY_FILENAME, DEFAULT_CONFIG, CONFIG_FILENAME, SUMMARY_COLLECTION_KEY_NAME
from tensorboard.plugins.uMAP import video_writing
from tensorboard.plugins.uMAP.visualizer import Visualizer
...
class uMAPHook(tf.train.SessionRunHook):
  """SessionRunHook implementation that runs uMAP every step.

  Convenient when using tf.train.MonitoredSession:
  ```python
  uMAP_hook = uMAPHook(LOG_DIRECTORY)
  with MonitoredSession(..., hooks=[uMAP_hook]) as sess:
    sess.run(train_op)
  ```
  """
  def __init__(self, logdir):
    ...
    Args:
      logdir: Directory where uMAP should write data.
    """
    self._logdir = logdir
    self.uMAP = None

  def begin(self):
    self.uMAP = uMAP(self._logdir)

  def after_run(self, run_context, unused_run_values):
    self.uMAP.update(run_context.session)
